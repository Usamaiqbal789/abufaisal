        ...
        def get_warehoueinfo(self):
            ...
            for w in warehoues:
                ...
                price_list_item = self.env['product.pricelist.item'].search(
                    [('pricelist_id', '=', price_list.id), ('product_id', '=', self.product_db_id.id)])
                cost = self.env['product.product'].search([('id', '=', self.product_db_id.id)])
                ...
                for l in locations:
                    ...
                    re_order = self.env['stock.warehouse.orderpoint'].search(
                        [('location_id', '=', l.id), ('product_id', '=', self.product_db_id.id)])
                    for ro in re_order:
                        if ro:
                            reorder_qty = str(ro.product_min_qty)
                            reorder_level = str(ro.product_max_qty)
            ...
            def get_supplier():
                po = self.env['purchase.order.line'].search([('product_id', '=', self.product_db_id.id)],
                                                            order='id asc')
                self.last_supplier = False
                self.supplier_reference = ""
                self.last_purchase_cost = 0
